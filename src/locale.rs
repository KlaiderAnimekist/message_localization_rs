...
pub fn parse_locale<S: ToString>(src: S) -> Result<Locale, String> {
    let src = src.to_string().replace("_", "-");
    ...
    if tag.get_region().is_none() {
        ...
        if src == "us" || src == "usa" { tag = LangTag::from_str("en_US").unwrap(); }
        if src == "jp" || src == "jpn" { tag = LangTag::from_str("ja").unwrap(); }
    }
...
impl Locale {
    ...
    pub fn direction(&self) -> Direction {
        ...
        if let Some(data) = data { data.d } else { Direction::LeftToRight }
    }

    pub fn international_name(&self) -> &str {
        let data = self._get_basic_info();
        if let Some(data) = data { &data.r#in } else { "" }
    }

    pub fn native_name(&self) -> &str {
        let data = self._get_basic_info();
        if let Some(data) = data { &data.nn } else { "" }
    }
...
impl Display for Locale {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        if let Some(country) = country {
            write!(f, "{} ({})", self.native_name(), country.international_name())
